...
class CommandInterpreter {
    ...
    Table selectClause() {
        ...
        Table table = tableName();
        if (_input.nextIf(",")) {
            Table second = tableName();
            conditions = conditionClause(table, second);
            result = table.select(second, list, conditions);
        } else {
            conditions = conditionClause(table);
            result = table.select(list, conditions);
        }
        return result;
    }
    ...
    Condition condition(Table... tables) {
        Column c1 = new Column(columnName(), tables);
        String relation = _input.next();
        Condition cond;
        if (_input.nextIs(Tokenizer.IDENTIFIER)) {
            Column c2 = new Column(columnName(), tables);
            cond = new Condition(c1, relation, c2);
        } else {
            cond = new Condition(c1, relation, literal());
        }
        return cond;
    }
