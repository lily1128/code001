...
(define (enumerate s)
  ...
  (define (helper n lst)
          (cond ((null? lst) nil)
                (else (cons (cons n (cons (car lst) nil)) (helper (+ n 1) (cdr lst))))
          )
  )
  (helper 0 s)
  )
  ...
  (cond ((atom? expr)
        ...
        ((let? expr)
         (let ((values (cadr expr))
           ...
           (define param_values (map let-to-lambda (cadr (zip values))))
           (define f (append (list 'lambda parameters) (map let-to-lambda body)))
           (cons f param_values)
           ; END PROBLEM 19
           ))
        (else
